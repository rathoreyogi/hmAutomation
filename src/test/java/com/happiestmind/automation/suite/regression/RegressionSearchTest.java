...
public class RegressionSearchTest extends RegressionSuiteBaseImpl {

	final static Logger LOG = Logger.getLogger(RegressionSearchTest.class);
	
	private final static String TESTCASE="Starting Regression Google Search Test";
	...
	public void checkTestSkip() {
		try {
			MiscUtils.beforeTestRunmodeCheckUp(regressionSuite, className);
			runmodes = TestUtil.getDataSetRunmodes(regressionSuite, className);
	...
	public void googleSearchRegressionTest(String browser, String uri, String searchWord, String title) throws Exception {
	...
	public void reportDataSetResult(ITestResult result) {
		...
		methodAssessment.setSuiteObject(regressionSuite);
	...
	public void reportTestResult() {
		...
		testAssessment.setSuiteObject(regressionSuite);
		try {
	...
	public Object[][] getTestData() throws IOException {
		try {
			return AutomationDataProvider.getData(regressionSuite, className);
		} catch (AutomationDataProviderCustomException e) {
			ExtentErrorHandler.dataProviderErrHandler(e, extent, extentLogger, TESTCASE, "RegressionSuite");
		}
		return null;
	}
